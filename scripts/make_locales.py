# -*- coding: utf-8 -*-
"""生成三语 locales —— 文案集中在这里，避免手写 JSON 漏键。

前端用扁平点号键（脚手架 i18n 直查），所以脚本产出就是扁平表；
`scripts/check_locales.py` 会与前端实际引用的键做对账。
"""
import json
import os

HERE = os.path.dirname(os.path.abspath(__file__))
LOCALES = os.path.join(HERE, '..', 'frontend', 'locales')

ZH = {
    'app.title': '查重合并',
    'app.footer': '查重判定为 clean-room 实现，设计思路参考 BookOrbit（AGPL-3.0）；判定规则与权重按 MyBooks 口径重做。',

    'scope.title': '查重范围',
    'scope.loading': '正在读取书库信息…',
    'scope.total': '共 {n} 本书',
    'scope.threshold': '相似度阈值',
    'scope.note': '说明',
    'scope.thresholdHint': '越高越保守（只报几乎一样的），越低越能查出改过书名的重复，但误报也更多。85 是默认值，结果里会显示实际使用的阈值，便于按你自己的书库调整。',
    'scope.start': '开始查重',
    'scope.cancel': '取消',
    'scope.failed': '读取书库信息失败',
    'scope.restoreFailed': '读取上次结果失败',
    'scope.startFailed': '启动失败',
    'scope.thresholdRange': '阈值应在 50~100 之间',

    'run.scanning': '正在查重…',
    'run.cancelled': '已请求取消',
    'run.failed': '查重失败',
    'run.idle': '没有正在进行的任务',
    'run.restored': '查重完成 · 上次查重结果',

    'phase.queued': '排队中',
    'phase.load': '正在读取书籍信息',
    'phase.compare': '正在比对标题与作者',
    'phase.done': '完成',

    'summary.title': '查重结果',
    'summary.groups': '重复分组',
    'summary.extra': '多余副本',
    'summary.reclaimable': '可回收空间',
    'summary.waste': '同格式重占',
    'summary.serialExcluded': '已排除卷册序号',
    'summary.crossTypeExcluded': '已排除实体/电子书交叉',
    'summary.ignoredExcluded': '已忽略的配对',

    'filter.all': '全部',
    'filter.confidence': '证据强度',
    'filter.keyword': '关键字',
    'filter.keywordPlaceholder': '书名或作者',

    'confidence.certain': '完全相同',
    'confidence.strong': '确定',
    'confidence.likely': '很可能',
    'confidence.weak': '存疑',

    'reason.isbn': 'ISBN 相同',
    'reason.fuzzy': '标题相似',
    'reason.sourceMissing': '这本书已不在书库',
    'reason.targetMissing': '保留项已不在书库',
    'reason.copyFailed': '格式复制失败',
    'reason.deleteFailed': '删除记录失败',
    'reason.sameBook': '源和保留项是同一本',
    'reason.weak': '仅标题相似',
    'reason.exact': '元数据完全相同',
    'reason.hash': '文件完全相同',

    'list.meta': '显示 {shown} / {total} 组 · 生成于 {at}',
    'list.titleJoin': '、',
    'list.membersMore': '等 {n} 本',
    'list.groupTitle': '{n} 本可能是同一本书',
    'list.staleHandled': '已处理 {n} 本',
    'list.sameFormat': '同格式 {v}',
    'list.failed': '读取列表失败',
    'list.emptyClean': '没有发现重复书籍',
    'list.emptyFiltered': '当前筛选条件下没有结果',

    'pager.prev': '上一页',
    'pager.next': '下一页',

    'bench.collapse': '收起',
    'bench.recommend': '推荐保留：{title}',
    'bench.yourPick': '保留这本：{title}',
    'bench.id': 'ID {id}',
    'bench.open': '打开书籍页',
    'bench.pick': '保留这本',
    'bench.picked': '已选为保留',
    'bench.keep': '保留',
    'bench.preview': '先看合并预览',
    'bench.pickMerge': '一并合并',
    'bench.ignore': '不是重复（忽略这组）',
    'bench.ignoreHint': '忽略只是"以后别再报出来"，不动书库、也不删记录；随时可以在「已忽略」里撤销。',
    'bench.ignoredPart': '本组有 {n} 对已被忽略：重新查重后会拆开。',

    'ignore.title': '已忽略（不是重复）',
    'ignore.count': '{n} 对',
    'ignore.hint': '这些配对以后查重不再报出来。撤销之后重新查重它们会再出现。',
    'ignore.item': '《{a}》 ↔ 《{b}》',
    'ignore.undo': '撤销',
    'ignore.undoAll': '全部撤销',
    'ignore.done': '已忽略《{titles}》：以后查重不再报出来（可在「已忽略」里撤销）',
    'ignore.undone': '已撤销 {n} 对忽略',
    'ignore.failed': '忽略失败',
    'ignore.undoFailed': '撤销失败',
    'bench.selectedCount': '已勾选 {n} 本将并入保留项（没勾的原样保留）',
    'bench.pickNoneHint': '还没勾选要合并的书：没勾的就原样保留，不会被动',
    'bench.selectAll': '全选',
    'bench.selectNone': '全不选',
    'bench.hint': '删掉源记录会连带失去它的收藏/在读/阅读进度/评分/书单，请先确认要保留哪一本。工具读不到这些数据，所以这一步必须由你判断。',
    'bench.failed': '读取分组失败',

    'diff.field': '对比项',
    'summary.serialExcluded': '已排除卷册序号',
    'summary.crossTypeExcluded': '已排除实体/电子书交叉',
    'summary.ignoredExcluded': '已忽略的配对',
    'diff.listJoin': '、',
    'diff.shared': '这些项两份都相同：{fields}',
    'diff.score': '{n} 分',
    'diff.stars': '{n} 星',
    'diff.unrated': '未评分',
    'diff.yes': '是',
    'diff.no': '否',
    'diff.physical': '实体书',
    'diff.ebook': '电子书',
    'diff.field.size': '体积',
    'diff.field.formats': '格式',
    'diff.field.isbn': 'ISBN',
    'diff.field.metadata': '元数据',
    'diff.field.cover': '封面',
    'diff.field.added': '入库时间',
    'diff.field.rating': '评分',
    'diff.field.tags': '标签',
    'diff.field.series': '系列',
    'diff.field.publisher': '出版社',
    'diff.field.languages': '语言',
    'diff.field.translators': '译者',
    'diff.field.collector': '收藏者',
    'diff.field.sole': '私有',
    'diff.field.book_type': '书目类型',
    'diff.allSame': '两份的元数据与条目属性完全相同',

    'keep.metadata': '元数据最全',
    'keep.withValue': '{text}（{value}）',
    'keep.formats': '格式最多',
    'keep.size': '体积最大',
    'keep.isbn': '有 ISBN',
    'keep.oldest': '最早入库',
    'keep.newest': '最新入库',
    'keep.added': '按入库时间',
    'keep.manual': '你手动选择',
    'keep.protected': '带阅读数据',
    'keep.multiProtected': '多份都带阅读数据，工具无法替你决定',

    'plan.title': '合并预览',
    'plan.keep': '保留：{title}',
    'plan.step': '《{src}》(#{id}) → 并入《{dst}》',
    'plan.moved': '会复制过去：',
    'plan.dropped': '会被丢弃：',
    'plan.none': '无',
    'plan.movedTotal': '复制格式 {n} 个',
    'plan.droppedTotal': '丢弃同格式 {n} 个',
    'plan.reclaim': '可回收 {v}',
    'plan.warnDrop': '同名格式不会被复制：源书的同名文件会被丢弃，留下的是保留项的那一份。',
    'plan.warnMigrate': '源记录的收藏/在读/阅读进度/评分/书单会随删除**一起消失**，不会迁移到保留项。',
    'plan.deleteSourceCount': '合并后删除勾选的 {n} 条记录',
    'plan.deleteSourceOff': '不勾则只把格式并过去：两条记录都留着（同名格式那一份仍会被丢弃，磁盘占用不会下降）。',
    'plan.trashHint': '删掉的书会进书库回收站（后台 → 回收站 可以恢复）；但这本书记的收藏/在读/进度/评分/书评/书单归属会一起消失，恢复也带不回来。',
    'plan.userDataWarn': '勾选要删的这几本里，{n} 本带着你的评分/标签/简介：{titles}',
    'plan.kept': '未勾选、保持原样 {n} 本：{titles}',
    'plan.applyCount': '确认合并（{n} 本）',
    'plan.cancel': '取消',
    'plan.applied': '已合并：复制 {m} 个格式，删除 {d} 条重复记录',
    'plan.appliedKept': '已合并：复制 {m} 个格式，删除 {d} 条重复记录；{k} 本未勾选、保持原样',
    'plan.appliedPartial': '已合并：复制 {m} 个格式，删除 {d} 条重复记录；{f} 条失败（详见「本次已处理」）',
    'plan.applyFailed': '合并失败',
    'plan.failed': '生成合并预览失败',

    'handled.title': '本次已处理',
    'handled.item': '《{title}》已并入《{into}》',
    'handled.deleted': '《{title}》已删除',
    'handled.failed': '《{title}》未处理：{reason}',
    'del.button': '删除',
    'del.title': '删除《{title}》？',
    'del.warn': '会连带删除这本书记的收藏/在读/阅读进度/时长/评分/书评/书单归属——这些都不会搬到同组的其它书上。',
    'del.confirm': '确认删除',
    'del.cancel': '取消',
    'del.applied': '已删除《{title}》',
    'del.failed': '删除失败',
}

EN = {
    'app.title': 'Duplicate Finder',
    'app.footer': 'Duplicate detection is a clean-room implementation; the design approach references BookOrbit (AGPL-3.0), with rules and weights redone for MyBooks.',

    'scope.title': 'Scope',
    'scope.loading': 'Reading library information…',
    'scope.total': '{n} books in library',
    'scope.threshold': 'Similarity threshold',
    'scope.note': 'Note',
    'scope.thresholdHint': 'Higher is stricter (nearly identical only); lower catches retitled duplicates but also more false positives. 85 is the default, and the value actually used is shown in the report so you can tune it for your own library.',
    'scope.start': 'Start scan',
    'scope.cancel': 'Cancel',
    'scope.failed': 'Could not read library information',
    'scope.restoreFailed': 'Could not read the previous result',
    'scope.startFailed': 'Could not start',
    'scope.thresholdRange': 'Threshold must be between 50 and 100',

    'run.scanning': 'Scanning…',
    'run.cancelled': 'Cancellation requested',
    'run.failed': 'Scan failed',
    'run.idle': 'No task in progress',
    'run.restored': 'Scan complete · previous result',

    'phase.queued': 'Queued',
    'phase.load': 'Reading book records',
    'phase.compare': 'Comparing titles and authors',
    'phase.done': 'Done',

    'summary.title': 'Result',
    'summary.groups': 'Duplicate groups',
    'summary.extra': 'Extra copies',
    'summary.reclaimable': 'Reclaimable',
    'summary.waste': 'Same-format overlap',
    'summary.serialExcluded': 'volume numbers excluded',
    'summary.crossTypeExcluded': 'physical/e-book pairs excluded',
    'summary.ignoredExcluded': 'pairs you ignored',

    'filter.all': 'All',
    'filter.confidence': 'Confidence',
    'filter.keyword': 'Keyword',
    'filter.keywordPlaceholder': 'Title or author',

    'confidence.certain': 'Identical',
    'confidence.strong': 'Certain',
    'confidence.likely': 'Likely',
    'confidence.weak': 'Uncertain',

    'reason.isbn': 'Same ISBN',
    'reason.fuzzy': 'Similar title',
    'reason.sourceMissing': 'this book is no longer in the library',
    'reason.targetMissing': 'the keeper is no longer in the library',
    'reason.copyFailed': 'copying formats failed',
    'reason.deleteFailed': 'deleting the record failed',
    'reason.sameBook': 'source and keeper are the same book',
    'reason.weak': 'Title only',
    'reason.exact': 'Identical metadata',
    'reason.hash': 'Identical file',

    'list.meta': 'Showing {shown} / {total} groups · generated {at}',
    'list.titleJoin': ', ',
    'list.membersMore': '+{n} more',
    'list.groupTitle': '{n} copies may be the same book',
    'list.staleHandled': '{n} already handled',
    'list.sameFormat': 'same format {v}',
    'list.failed': 'Could not read the list',
    'list.emptyClean': 'No duplicate books found',
    'list.emptyFiltered': 'No groups match the current filter',

    'pager.prev': 'Previous',
    'pager.next': 'Next',

    'bench.collapse': 'Collapse',
    'bench.recommend': 'Recommended to keep: {title}',
    'bench.yourPick': 'Keeping: {title}',
    'bench.id': 'ID {id}',
    'bench.open': 'Open book page',
    'bench.pick': 'Keep this one',
    'bench.picked': 'Selected to keep',
    'bench.keep': 'Keep',
    'bench.preview': 'Review merge first',
    'bench.pickMerge': 'Merge this one too',
    'bench.ignore': 'Not duplicates (ignore this group)',
    'bench.ignoreHint': 'Ignoring means "stop reporting these" — nothing is written to the library and no record is deleted. Undo it anytime under "Ignored".',
    'bench.ignoredPart': '{n} pair(s) in this group are already ignored: a rescan will split it.',

    'ignore.title': 'Ignored (not duplicates)',
    'ignore.count': '{n} pair(s)',
    'ignore.hint': 'These pairs will no longer be reported. Undo one and the next scan will show it again.',
    'ignore.item': '《{a}》 ↔ 《{b}》',
    'ignore.undo': 'Undo',
    'ignore.undoAll': 'Undo all',
    'ignore.done': 'Ignored 《{titles}》: no longer reported (undo under "Ignored")',
    'ignore.undone': 'Undid {n} ignored pair(s)',
    'ignore.failed': 'Could not ignore',
    'ignore.undoFailed': 'Could not undo',
    'bench.selectedCount': '{n} selected to merge into the kept copy (unchecked copies stay as they are)',
    'bench.pickNoneHint': 'Nothing selected yet: unchecked copies are left untouched',
    'bench.selectAll': 'Select all',
    'bench.selectNone': 'Select none',
    'bench.hint': 'Deleting a source record also loses its favorites, reading status, progress, rating and booklists. Confirm which copy to keep first — the tool cannot read that data, so this call is yours.',
    'bench.failed': 'Could not read the group',

    'diff.field': 'Field',
    'summary.serialExcluded': 'volume numbers excluded',
    'summary.crossTypeExcluded': 'physical/e-book pairs excluded',
    'summary.ignoredExcluded': 'pairs you ignored',
    'diff.listJoin': ', ',
    'diff.shared': 'Identical in both copies: {fields}',
    'diff.score': '{n} pts',
    'diff.stars': '{n} stars',
    'diff.unrated': 'Not rated',
    'diff.yes': 'Yes',
    'diff.no': 'No',
    'diff.physical': 'Physical book',
    'diff.ebook': 'E-book',
    'diff.field.size': 'Size',
    'diff.field.formats': 'Formats',
    'diff.field.isbn': 'ISBN',
    'diff.field.metadata': 'Metadata',
    'diff.field.cover': 'Cover',
    'diff.field.added': 'Added',
    'diff.field.rating': 'Rating',
    'diff.field.tags': 'Tags',
    'diff.field.series': 'Series',
    'diff.field.publisher': 'Publisher',
    'diff.field.languages': 'Languages',
    'diff.field.translators': 'Translators',
    'diff.field.collector': 'Collector',
    'diff.field.sole': 'Private',
    'diff.field.book_type': 'Book type',
    'diff.allSame': 'Both copies have identical metadata and record attributes',

    'keep.metadata': 'Most complete metadata',
    'keep.withValue': '{text} ({value})',
    'keep.formats': 'Most formats',
    'keep.size': 'Largest',
    'keep.isbn': 'Has ISBN',
    'keep.oldest': 'Added earliest',
    'keep.newest': 'Added latest',
    'keep.added': 'By added time',
    'keep.manual': 'Your choice',
    'keep.protected': 'Carries reading data',
    'keep.multiProtected': 'Several copies carry reading data — the tool cannot decide',

    'plan.title': 'Merge preview',
    'plan.keep': 'Keep: {title}',
    'plan.step': '《{src}》 (#{id}) → merged into 《{dst}》',
    'plan.moved': 'Will be copied over: ',
    'plan.dropped': 'Will be discarded: ',
    'plan.none': 'none',
    'plan.movedTotal': '{n} formats copied',
    'plan.droppedTotal': '{n} same formats discarded',
    'plan.reclaim': '{v} reclaimable',
    'plan.warnDrop': 'Same-name formats are not copied: the source copy of those files is discarded and the kept copy wins.',
    'plan.warnMigrate': 'Favorites, reading status, progress, ratings and booklists of the source record are **deleted with it** — they are not migrated to the keeper.',
    'plan.deleteSourceCount': 'Delete the {n} selected records after merging',
    'plan.deleteSourceOff': 'Unchecked: formats are merged over only — both records stay (same-name copies are still discarded and disk usage does not drop).',
    'plan.trashHint': 'Deleted books go to the library recycle bin (Admin → Recycle bin can restore them); but this copy\'s favorites, reading status, progress, rating, review and booklist membership are gone for good.',
    'plan.userDataWarn': 'Of the records selected for deletion, {n} carry your rating/tags/summary: {titles}',
    'plan.kept': '{n} left unchecked and untouched: {titles}',
    'plan.applyCount': 'Confirm merge ({n})',
    'plan.cancel': 'Cancel',
    'plan.applied': 'Merged: {m} formats copied, {d} duplicate records deleted',
    'plan.appliedKept': 'Merged: {m} formats copied, {d} duplicate records deleted; {k} left unchecked and untouched',
    'plan.appliedPartial': 'Merged: {m} formats copied, {d} duplicates deleted; {f} failed (see "Handled in this session")',
    'plan.applyFailed': 'Merge failed',
    'plan.failed': 'Could not build the merge preview',

    'handled.title': 'Handled in this session',
    'handled.item': '《{title}》 merged into 《{into}》',
    'handled.deleted': '《{title}》 deleted',
    'handled.failed': '《{title}》 not handled: {reason}',
    'del.button': 'Delete',
    'del.title': 'Delete 《{title}》?',
    'del.warn': 'This also removes the favorites, reading status, progress, duration, rating, review and booklist membership recorded on this copy — none of it moves to the other copies in the group.',
    'del.confirm': 'Delete',
    'del.cancel': 'Cancel',
    'del.applied': 'Deleted 《{title}》',
    'del.failed': 'Delete failed',
}

ZH_TW = {
    'app.title': '查重合併',
    'app.footer': '查重判定為 clean-room 實作，設計思路參考 BookOrbit（AGPL-3.0）；判定規則與權重依 MyBooks 口徑重做。',

    'scope.title': '查重範圍',
    'scope.loading': '正在讀取書庫資訊…',
    'scope.total': '共 {n} 本書',
    'scope.threshold': '相似度閾值',
    'scope.note': '說明',
    'scope.thresholdHint': '越高越保守（只報幾乎一樣的），越低越能查出改過書名的重複，但誤報也更多。85 是預設值，結果裡會顯示實際使用的閾值，方便依自己的書庫調整。',
    'scope.start': '開始查重',
    'scope.cancel': '取消',
    'scope.failed': '讀取書庫資訊失敗',
    'scope.restoreFailed': '讀取上次結果失敗',
    'scope.startFailed': '啟動失敗',
    'scope.thresholdRange': '閾值應在 50~100 之間',

    'run.scanning': '正在查重…',
    'run.cancelled': '已請求取消',
    'run.failed': '查重失敗',
    'run.idle': '沒有正在進行的任務',
    'run.restored': '查重完成 · 上次查重結果',

    'phase.queued': '排隊中',
    'phase.load': '正在讀取書籍資訊',
    'phase.compare': '正在比對標題與作者',
    'phase.done': '完成',

    'summary.title': '查重結果',
    'summary.groups': '重複分組',
    'summary.extra': '多餘副本',
    'summary.reclaimable': '可回收空間',
    'summary.waste': '同格式重佔',
    'summary.serialExcluded': '已排除卷冊序號',
    'summary.crossTypeExcluded': '已排除實體／電子書交叉',
    'summary.ignoredExcluded': '已忽略的配對',

    'filter.all': '全部',
    'filter.confidence': '證據強度',
    'filter.keyword': '關鍵字',
    'filter.keywordPlaceholder': '書名或作者',

    'confidence.certain': '完全相同',
    'confidence.strong': '確定',
    'confidence.likely': '很可能',
    'confidence.weak': '存疑',

    'reason.isbn': 'ISBN 相同',
    'reason.fuzzy': '標題相似',
    'reason.sourceMissing': '這本書已不在書庫',
    'reason.targetMissing': '保留項已不在書庫',
    'reason.copyFailed': '格式複製失敗',
    'reason.deleteFailed': '刪除記錄失敗',
    'reason.sameBook': '來源和保留項是同一本',
    'reason.weak': '僅標題相似',
    'reason.exact': '後設資料完全相同',
    'reason.hash': '檔案完全相同',

    'list.meta': '顯示 {shown} / {total} 組 · 產生於 {at}',
    'list.titleJoin': '、',
    'list.membersMore': '等 {n} 本',
    'list.groupTitle': '{n} 本可能是同一本書',
    'list.staleHandled': '已處理 {n} 本',
    'list.sameFormat': '同格式 {v}',
    'list.failed': '讀取列表失敗',
    'list.emptyClean': '沒有發現重複書籍',
    'list.emptyFiltered': '目前篩選條件下沒有結果',

    'pager.prev': '上一頁',
    'pager.next': '下一頁',

    'bench.collapse': '收合',
    'bench.recommend': '建議保留：{title}',
    'bench.yourPick': '保留這本：{title}',
    'bench.id': 'ID {id}',
    'bench.open': '開啟書籍頁',
    'bench.pick': '保留這本',
    'bench.picked': '已選為保留',
    'bench.keep': '保留',
    'bench.preview': '先看合併預覽',
    'bench.pickMerge': '一併合併',
    'bench.ignore': '不是重複（忽略這組）',
    'bench.ignoreHint': '忽略只是「以後別再報出來」，不動書庫、也不刪記錄；隨時可以在「已忽略」裡撤銷。',
    'bench.ignoredPart': '本組有 {n} 對已被忽略：重新查重後會拆開。',

    'ignore.title': '已忽略（不是重複）',
    'ignore.count': '{n} 對',
    'ignore.hint': '這些配對以後查重不再報出來。撤銷之後重新查重它們會再出現。',
    'ignore.item': '《{a}》 ↔ 《{b}》',
    'ignore.undo': '撤銷',
    'ignore.undoAll': '全部撤銷',
    'ignore.done': '已忽略《{titles}》：以後查重不再報出來（可在「已忽略」裡撤銷）',
    'ignore.undone': '已撤銷 {n} 對忽略',
    'ignore.failed': '忽略失敗',
    'ignore.undoFailed': '撤銷失敗',
    'bench.selectedCount': '已勾選 {n} 本將併入保留項（沒勾的原樣保留）',
    'bench.pickNoneHint': '還沒勾選要合併的書：沒勾的就原樣保留，不會被動',
    'bench.selectAll': '全選',
    'bench.selectNone': '全不選',
    'bench.hint': '刪掉來源記錄會一併失去它的收藏／在讀／閱讀進度／評分／書單，請先確認要保留哪一本。工具讀不到這些資料，所以這一步必須由你判斷。',
    'bench.failed': '讀取分組失敗',

    'diff.field': '對照項',
    'summary.serialExcluded': '已排除卷冊序號',
    'summary.crossTypeExcluded': '已排除實體／電子書交叉',
    'summary.ignoredExcluded': '已忽略的配對',
    'diff.listJoin': '、',
    'diff.shared': '這些項兩份都相同：{fields}',
    'diff.score': '{n} 分',
    'diff.stars': '{n} 星',
    'diff.unrated': '未評分',
    'diff.yes': '是',
    'diff.no': '否',
    'diff.physical': '實體書',
    'diff.ebook': '電子書',
    'diff.field.size': '體積',
    'diff.field.formats': '格式',
    'diff.field.isbn': 'ISBN',
    'diff.field.metadata': '中繼資料',
    'diff.field.cover': '封面',
    'diff.field.added': '入庫時間',
    'diff.field.rating': '評分',
    'diff.field.tags': '標籤',
    'diff.field.series': '系列',
    'diff.field.publisher': '出版社',
    'diff.field.languages': '語言',
    'diff.field.translators': '譯者',
    'diff.field.collector': '收藏者',
    'diff.field.sole': '私有',
    'diff.field.book_type': '書目類型',
    'diff.allSame': '兩份的後設資料與條目屬性完全相同',

    'keep.metadata': '後設資料最完整',
    'keep.withValue': '{text}（{value}）',
    'keep.formats': '格式最多',
    'keep.size': '體積最大',
    'keep.isbn': '有 ISBN',
    'keep.oldest': '最早入庫',
    'keep.newest': '最新入庫',
    'keep.added': '依入庫時間',
    'keep.manual': '你手動選擇',
    'keep.protected': '帶閱讀資料',
    'keep.multiProtected': '多份都帶閱讀資料，工具無法替你決定',

    'plan.title': '合併預覽',
    'plan.keep': '保留：{title}',
    'plan.step': '《{src}》(#{id}) → 併入《{dst}》',
    'plan.moved': '會複製過去：',
    'plan.dropped': '會被丟棄：',
    'plan.none': '無',
    'plan.movedTotal': '複製格式 {n} 個',
    'plan.droppedTotal': '丟棄同格式 {n} 個',
    'plan.reclaim': '可回收 {v}',
    'plan.warnDrop': '同名格式不會被複製：來源書的同名檔案會被丟棄，留下的是保留項的那一份。',
    'plan.warnMigrate': '來源記錄的收藏／在讀／閱讀進度／評分／書單會隨刪除**一起消失**，不會移轉到保留項。',
    'plan.deleteSourceCount': '合併後刪除勾選的 {n} 筆記錄',
    'plan.deleteSourceOff': '不勾則只把格式併過去：兩筆記錄都留著（同名格式那一份仍會被丟棄，磁碟佔用不會下降）。',
    'plan.trashHint': '刪掉的書會進書庫回收站（後台 → 回收站 可以復原）；但這本書記的收藏／在讀／進度／評分／書評／書單歸屬會一起消失，復原也帶不回來。',
    'plan.userDataWarn': '勾選要刪的這幾本裡，{n} 本帶著你的評分／標籤／簡介：{titles}',
    'plan.kept': '未勾選、保持原樣 {n} 本：{titles}',
    'plan.applyCount': '確認合併（{n} 本）',
    'plan.cancel': '取消',
    'plan.applied': '已合併：複製 {m} 個格式，刪除 {d} 筆重複記錄',
    'plan.appliedKept': '已合併：複製 {m} 個格式，刪除 {d} 筆重複記錄；{k} 本未勾選、保持原樣',
    'plan.appliedPartial': '已合併：複製 {m} 個格式，刪除 {d} 筆重複記錄；{f} 筆失敗（詳見「本次已處理」）',
    'plan.applyFailed': '合併失敗',
    'plan.failed': '產生合併預覽失敗',

    'handled.title': '本次已處理',
    'handled.item': '《{title}》已併入《{into}》',
    'handled.deleted': '《{title}》已刪除',
    'handled.failed': '《{title}》未處理：{reason}',
    'del.button': '刪除',
    'del.title': '刪除《{title}》？',
    'del.warn': '會一併刪除這本書記的收藏／在讀／閱讀進度／時長／評分／書評／書單歸屬——這些都不會搬到同組的其它書上。',
    'del.confirm': '確認刪除',
    'del.cancel': '取消',
    'del.applied': '已刪除《{title}》',
    'del.failed': '刪除失敗',
}


def main():
    os.makedirs(LOCALES, exist_ok=True)
    payloads = {'zh': ZH, 'en': EN, 'zh-TW': ZH_TW}
    base = set(ZH)
    for code, table in payloads.items():
        missing = base - set(table)
        extra = set(table) - base
        if missing or extra:
            raise SystemExit('%s 键不一致：缺 %s / 多 %s' % (code, sorted(missing), sorted(extra)))
        path = os.path.join(LOCALES, code + '.json')
        # `newline='\n'` 不能省：Windows 上默认会把 \n 翻成 \r\n，产物字节就与 clone 里
        # 打出来的不一样（`scripts/build.py` 的 CRLF 守门正是为这个设的）。
        with open(path, 'w', encoding='utf-8', newline='\n') as handle:
            json.dump(table, handle, ensure_ascii=False, indent=2, sort_keys=True)
            handle.write('\n')
        print('wrote %s (%d keys)' % (os.path.relpath(path, HERE), len(table)))


if __name__ == '__main__':
    main()