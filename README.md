...
2. **看结果**：每个分组显示证据强度（确定 / 很可能 / 存疑）、成员数、可回收空间。
   点一行展开**对照表**——只列出成员之间**有差异**的字段，相同的折叠成一句摘要。
3. **选保留哪本**：工具按规则给推荐（元数据最全 → 格式最多 → 体积最大 → 最早入库），
   并写明理由；你可以在对照卡上点「保留这本」覆盖它。
4. **合并**：点「先看合并预览」，确认后执行。预览会列出**会复制过去的格式**与
   **会被丢弃的同名格式**——同名格式不会被复制（见下）。
